...
TEST_F(TpccPlanGenerateTests, GenerateAll) {
  ...
  std::string query;
  std::string tbl;
  std::ofstream rec("tpcc_files/finished_list.txt");

  for (int i = 0; i < test_cnt_use; i++) {
    std::ifstream ifs_sql("tpcc_files/raw/" + names_use[i] + ".sql");
    std::ifstream ifs_tbl("tpcc_files/raw/" + names_use[i] + ".txt");
    int query_cnt = 0;
    ...
    while (getline(ifs_sql, query)) {
      ...
      std::ofstream ofs("tpcc_files/json/" + names_use[i] + "/" + std::to_string(query_cnt) + ".json");
